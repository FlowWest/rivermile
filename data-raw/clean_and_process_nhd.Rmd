...
```
```{r}
klamath_hucs <- hucs |> select(huc8, name, geometry)

usethis::use_data(klamath_hucs, overwrite = TRUE)
```
...
external_crosswalk <- st_read(here::here('data-raw', 'shapefiles', 'NHDPLUS_H_1801_HU4_GDB.gdb'), layer = 'ExternalCrosswalk') |> 
  janitor::clean_names() 
...
usethis::use_data(nhd_flowline, overwrite = TRUE)

```

#### Get Catchments

source: https://www.epa.gov/waterdata/get-nhdplus-national-hydrography-dataset-plus-data
```{r}
layers <- st_layers(here::here("data-raw", "shapefiles", "nhdplus_epasnapshot2022_or.gdb"))

catchment_fabric_or <- st_read(here::here("data-raw", "shapefiles", "nhdplus_epasnapshot2022_or.gdb"), layer =  "catchment_fabric_or")

layers <- st_layers(here::here("data-raw", "shapefiles", "18", "NHDPLUS_H_1801_HU4_GDB.gdb"))

catchment_ca <- st_read(here::here("data-raw", "shapefiles", "18", "NHDPLUS_H_1801_HU4_GDB.gdb"), layer =  "NHDPlusCatchment")
...
rivers <- c('Klamath River', 'Trinity River', 'Sprague River',
            ...
            'Spencer Creek', 'Shovel Creek',' Fall Creek', 'Camp Creek', 'Scotch Creek')
...
```{r}
rivers_to_add <- c('Spencer Creek', 'Shovel Creek','Fall Creek', 'Camp Creek', 'Scotch Creek')

ids <- c('01154962', '01118498', '00223366', '00260026', '00260027') 

nhd_flowline_filtered_new_rivers <- nhd_flowline |> 
  filter(gnis_name %in% rivers_to_add) |> 
  st_transform(crs = "+proj=longlat +datum=WGS84") |> 
  st_zm() |> 
  filter(!(gnis_id %in% c(ids))) |> 
  st_set_geometry('shape') |> 
  rename(geometry = shape)

leaflet(data = nhd_flowline_filtered_new_rivers) |> 
  addTiles() |> 
  addPolylines(popup = ~paste0("name: ", gnis_name, "<br>",
                               "id: ", gnis_id))


nhd_flowline_filtered <- bind_rows(nhd_flowline_filtered, nhd_flowline_filtered_new_rivers)
```
...
names(nhd_flowline_filtered_new_rivers)[
  names(nhd_flowline_filtered_new_rivers) != attr(nhd_flowline_filtered_new_rivers, "sf_column")
] <- names(processed_nhd)[
  names(processed_nhd) != attr(processed_nhd, "sf_column")
]

processed <- processed_nhd |> 
  bind_rows(nhd_flowline_filtered_new_rivers)

processed_nhd <- processed
usethis::use_data(processed_nhd, overwrite = TRUE)
...
```{r}
nhd_catchment_ids <- processed_nhd |> 
  rename(nhd_plus_id = nhd_pl_) |> 
  pull(nhd_plus_id) |> unique()

catchments_or <- catchment_fabric_or |> filter(nhdplusid %in% nhd_catchment_ids)

catchments_klamath_ca <- catchment_ca |> 
  filter(NHDPlusID %in% nhd_catchment_ids) |> 
 rename(nhdplusid = NHDPlusID, 
        areasqkm = AreaSqKm) |> 
  select(nhdplusid, areasqkm) |> 
  st_set_geometry("SHAPE") |> 
  rename(geometry = SHAPE) 

nhd_catchments <- catchments_or |> 
  select(nhdplusid, areasqkm) |> 
  st_set_geometry("Shape") |> 
  rename(geometry = Shape) |> 
  bind_rows(catchments_klamath_ca)

usethis::use_data(nhd_catchments, overwrite = TRUE)
...
all_klamath_rivers_line <- processed |> 
...
blue_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Blue Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
...
bogus_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                          "Bogus Creek",
                                          interval_m = 10,
                                          downstream_end = "end",
                                          crs_proj = 32610)
...
clear_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                          "Clear Creek",
                                          interval_m = 10,
                                          downstream_end = "end",
                                          crs_proj = 32610)
...
indian_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                           "Indian Creek",
                                           interval_m = 10,
                                           downstream_end = "end",
                                           crs_proj = 32610)
...
jenny_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                          "Jenny Creek",
                                          interval_m = 10,
                                          downstream_end = "end",
                                          crs_proj = 32610)
...
link_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Link River",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
...
klamath_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                            "Klamath River",
                                            interval_m = 10,
                                            downstream_end = "end",
                                            crs_proj = 32610)
...
lost_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Lost River",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
...
salmon_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                           "Salmon River",
                                           interval_m = 10,
                                           downstream_end = "end",
                                           crs_proj = 32610)
...
scott_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                          "Scott River",
                                          interval_m = 10,
                                          downstream_end = "end",
                                          crs_proj = 32610)
...
shasta_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                           "Shasta River",
                                           interval_m = 10,
                                           downstream_end = "end",
                                           crs_proj = 32610)
...
sprague_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                            "Sprague River",
                                            interval_m = 10,
                                            downstream_end = "end",
                                            crs_proj = 32610)
...
trinity_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                            "Trinity River",
                                            interval_m = 10,
                                            downstream_end = "end",
                                            crs_proj = 32610)
...
williamson_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                               "Williamson River",
                                               interval_m = 10,
                                               downstream_end = "end",
                                               crs_proj = 32610)
...
wood_river <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Wood River",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
...
# new rivers
spencer_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Spencer Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
usethis::use_data(spencer_creek, overwrite = TRUE)

shovel_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Shovel Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
usethis::use_data(shovel_creek, overwrite = TRUE)

fall_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Fall Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
usethis::use_data(fall_creek, overwrite = TRUE)

camp_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Camp Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
usethis::use_data(camp_creek, overwrite = TRUE)

scotch_creek <- rivermile::get_river_miles(all_klamath_rivers_line, 
                                         "Scotch Creek",
                                         interval_m = 10,
                                         downstream_end = "end",
                                         crs_proj = 32610)
usethis::use_data(scotch_creek, overwrite = TRUE)
...
point_layers <- list(wood_river, williamson_river, trinity_river, sprague_river, 
                     ...
                     spencer_creek, shovel_creek, fall_creek, camp_creek, scotch_creek)  
...
```{r}
temperature <- klamathWaterData::temperature_gage |> glimpse()
```
